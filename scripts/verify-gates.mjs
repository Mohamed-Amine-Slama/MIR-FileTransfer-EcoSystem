...
const GATES = [
  ...
  ['P4.5', 'Rate limiting + anomaly alerts', 'partial', 'limiter NOW ENFORCING over HTTP on claim-token/claim/upload-init (was tested but unreferenced - the API had NO rate limiting); memory store is per-instance, needs Redis before >1 replica; anomaly sweep unscheduled, no alert delivery'],
  ...
  ['P13', 'Log scrubbing + tracing', 'partial', 'scrubber NOW IN THE LOG PATH and tracer NOW EMITTING SPANS (both were tested but uncalled) - proven over real HTTP; route-templated span names, errors scrubbed; no Sentry SDK exists, so the gate\'s Sentry half is untestable'],
